...
//             return res.apiResponse(false, 'FoodEat details are missing', {}, 400);
//         }
//         const checkTitle = await FoodEatCategory.findOne({ title: title })
//         if (checkTitle) {
...
//         }
//         console.log('coming')
//         // const result = await uploadToCloudinary(req.file, 'foodEatCategory');
//         // console.log('result', result)
//         const uniqueId = `FoodEatCat-${moment().format('DDMMYYYYHHmmss')}`;
...
//                     const maxDoc = await FoodEatCategory.findOne({ momType: type }).sort({ index: -1 });
//                     const nextIndex = maxDoc ? maxDoc.index + 1 : 0;
//                     const { secure_url, public_id } = await uploadToCloudinary(req.file, 'foodEatCategory');

//                     categories.push(new FoodEatCategory({
//                         title,
//                         color,
//                         status,
//                         file: secure_url,
...
exports.add = async (req, res, next) => {
  try {
    ...
    const checkTitle = await FoodEatCategory.findOne({ title });
    if (checkTitle) {
      return res.apiResponse(false, 'Title already exists', {}, 400);
    }
...
exports.list = async (req, res, next) => {
    try {
        ...
        if (pagination === "true") {
            ...
            FoodEatCategory.paginate(match, options, function (err, data) {
                if (err) {
            ...
            const query = Object.keys(match).length === 0
                ? FoodEatCategory.find({})
                : FoodEatCategory.find(match);
...
exports.view = async (req, res, next) => {
    try {
        ...
        const category = await FoodEatCategory.findOne({ id: requests.id })
        if (!category) {
...
exports.update = async (req, res, next) => {
    try {
        if (req.body) {
            ...
            const updateFields = {};
            if (req.body.title) {
                const checkTitle = await FoodEatCategory.findOne({ title: req.body.title, momType: req.body.momType })
            ...
            const updatedCategory = await FoodEatCategory.findOneAndUpdate(
                { id: id },
                { $set: updateFields },
                { new: true }
            );
...
exports.delete = async (req, res, next) => {
    try {
        ...
        const result = await FoodEatCategory.deleteOne({ id: requests.id });
...
//     try {
//         const docs = await FoodEatCategory.find({});
...
//             }
//         }));
//         await FoodEatCategory.bulkWrite(bulkOps);
//         res.status(200).json({ message: "Indexes updated using bulkWrite." });
//     } catch (error) {
...
exports.updateIndex = async (req, res, next) => {
    try {
        ...
        const itemToMove = await FoodEatCategory.findOne({ id });
        if (!itemToMove) {
        ...
        // const checkIndex = await FoodEatCategory.findOne({ index: newIndex, momType: itemToMove.momType });
        // if (!checkIndex) {
        //     return res.status(400).json({ message: `Index ${newIndex} not available to swap.` });
        // }
        const targetDoc = await FoodEatCategory.findOne({ index: newIndex, momType: itemToMove.momType });
        if (!targetDoc) {
