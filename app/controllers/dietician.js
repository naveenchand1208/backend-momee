...
exports.add = async (req, res, next) => {
    try {
        const { name, email, dob, mobile, gender, qualification, experince } = req.body;
        if (!name || !email || !dob || !mobile || !gender || !qualification || !experince || !req.certificates) {
        ...
        // }
        const uploadedFiles = [];
        for (const file of req.certificates) {
            const result = await uploadToCloudinary(file, 'dieticianCertificate');
            uploadedFiles.push({
        ...
        const uniqueId = `Dietician-${moment().format('DDMMYYYYHHmmss')}`;
