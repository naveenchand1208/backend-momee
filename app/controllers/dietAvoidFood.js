const DietAvoidFood = require('../models/dietAvoidFood')
...
exports.add = async (req, res, next) => {
    try {
        const { userId, templateId, categoryId, description, symptoms } = req.bodyParams;
        if (!userId || !templateId || !categoryId || !description || symptoms.length === 0) {
        ...
        const category = await FoodAvoidCategory.findOne({ id: categoryId });
        if (!category) {
        ...
        const uniqueId = `DietAvoidFood-${moment().format('DDMMYYYYHHmmss')}`;
        const newDietAvoidFood = new DietAvoidFood({
            id: uniqueId,
            userId,
            templateId,
            categoryId,
        ...
        await newDietAvoidFood.save();
        return res.apiResponse(true, "Foods added successfully", newDietAvoidFood, 200);
    } catch (error) {
        console.error(error);
...
exports.list = async (req, res, next) => {
    try {
        ...
        //     DietAvoidFood.paginate(match, options, function (err, data) {
        //         if (err) {
        ...
        //         ? DietAvoidFood.find({})
        //         : DietAvoidFood.find(match);
        //     // foods = await query.sort({ createdAt: -1 });
        //      foods = await query
        //         .sort({ createdAt: 1 })
        //         .populate([
        ...
        //             { path: 'template' },
        //         ]);
        //     await DietAvoidFood.populate(query, [
        //         {
        //             path: 'user',
        //             select: 'userName id'
        //         },
...
exports.view = async (req, res, next) => {
    try {
        ...
        const food = await DietAvoidFood.findOne({ id: requests.id })
        if (!food) {
...
exports.update = async (req, res, next) => {
    try {
        ...
        const category = await FoodAvoidCategory.findOne({ id: requests.categoryId });
        if (!category) {
            return res.apiResponse(false, 'Category Not Found', {}, 404);
        }
        ...
        const food = await DietAvoidFood.findOneAndUpdate(
            { id: requests.id },
            updateFields,
            { new: true }
        );
...
exports.delete = async (req, res, next) => {
    try {
        ...
        const result = await DietAvoidFood.deleteOne({ id: requests.id });
...
exports.dateWiseFoodList = async (req, res, next) => {
    try {
        ...
        // Fetch all docs
        let docs = await DietAvoidFood.find({})
            .populate([
                { path: 'user', select: 'userName id' },
                { path: 'category', select: 'title id' },
                { path: 'template' }
