...
// Register BEFORE routes/models are required so every Mongoose schema gets translations.
mongoose.plugin(bilingualPlugin);
const compression = require('compression');
const languageMiddleware = require('./middleware/language');
const { localizePayload } = require('./helpers/localizeResponse');

const autoTranslateResponse =
  require('./middleware/autoTranslateResponse');
...
// Language selection must run after res.apiResponse exists.
app.use(languageMiddleware);
app.use(autoTranslateResponse);

// Localize every JSON response (including controllers that still use hard-coded messages).
app.use((req, res, next) => {
  const originalJson = res.json.bind(res);
  res.json = (body) => {
    Promise.resolve(localizePayload(body, req))
